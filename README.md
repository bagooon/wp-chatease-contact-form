...
管理画面 → **設定 → ChatEase Contact Form** で次の項目を設定します。

- ChatEase API トークン
- Workspace Slug
- 通知先メールアドレス（グローバルデフォルト）
- reCAPTCHA v2 Site Key
- reCAPTCHA v2 Secret Key
- 返答期限（日数）（グローバルデフォルト、例：1）
...
1. 管理画面左メニュー → **ChatEase フォーム** → 新規追加
2. フォームごとに以下を設定できます：
   - フォーム名（投稿タイトル）
   - ラベル類
     - 会社名ラベル
...
> フォームごとの設定を空欄にした場合は、  
> グローバル設定 → さらに空なら WordPress 管理者メール  
> といった優先順位で利用されます。
...
作成した `ChatEase フォーム` の投稿 ID を確認し（URL の `post=123` の数字）、  
固定ページや投稿本文にショートコードを埋め込みます：
