...
- フロント：
  - 会社名 / お名前 / メールアドレス / 電話番号 / お問い合わせ内容 のシンプルなフォーム
  - 入力 → 確認 → 完了 の 3 ステップ
  - reCAPTCHA v2（チェックボックス）対応
  - フォームごとにプラグイン同梱CSSの適用 ON/OFF が可能
- バックエンド：
  - ChatEase API を使ってチャットボードを自動生成
  - フォームごとにチャットボードタイトルを設定可能（未設定時は「フォームからのお問い合わせ」）
...
2. フォームごとに以下を設定できます：
   ...
   - ラベル類
     ...
     - お問い合わせ内容ラベル
   - 電話番号入力欄の表示 ON/OFF
   - チャットボードタイトル（未設定時: `フォームからのお問い合わせ`）
   - プラグイン同梱スタイルを適用（ON/OFF）
   - フォーム固有の返答期限（日数）
   - フォーム固有の通知先メールアドレス
...
- メモ：
    - 電話番号欄が有効かつ入力されている場合、`memo` に `TEL: 000-0000-0000` の形式で連携
- ステータス：
